...
- 新增 Auto Agent、自主激活、Todo、Dreaming、Experience 与文件系统手册。
- 完善 Agent 工作区、记忆生命周期、Trigger 条件审批和 Node→Manage 反馈闭环。
- 统一 Node/Manage UI 视觉系统，清理退役兼容路径并强化升级安全边界。

### 受控自主任务与用户反馈

- 新增普通/Auto Agent 类型。自主任务在智能体设置中配置，复用本 Agent 的模型、工具权限和专用执行会话；空闲时可正常聊天，并通过受限工具调整目标、完成条件、后续时间或暂停。
- 自主任务统一从 `PUT /v1/agents/{agent_id}/autonomy` 创建；旧 `POST /v1/goals` 返回迁移提示。普通 Agent 不能通过旧 Goal 接口启动自主执行，历史记录保留查看与停止。
- 长期任务包含检查点、完成证据、累计用量、调度与运行历史；忙碌时延后，重启未知运行需人工核对。当前每个 Auto Agent 仅支持一份任务，终态后的新周期复用尚未实现；Token 阈值不是精确费用硬上限。
- Node 反馈本地保存并向创建时绑定的 Manage 投递，支持重试、送达状态、管理员回复同步；Manage 提供筛选、处理与并发更新保护。
- Manage 保护接口默认要求鉴权，移除默认管理员密码并使旧不安全会话失效。升级前按 `manage/README.md` 配置管理员凭据。
- Trigger 投递绑定目标 Agent 与会话，持久化投递身份并保护并发领取；移除任意 shell 条件门控，未知投递提供人工恢复入口。
- 两端设置、反馈、案例资源与运行记录布局统一，修复窄屏导航挤压、长工具名溢出及 Auto 状态条主题。
...
工作组交互与本地桌面控制增强版：补齐成员询问恢复入口，并增加真实屏幕截图和受审批保护的桌面操作能力。

### 新增与优化

- 工作组成员调用 `ask_user_information` 时展示可输入的问答卡片，支持选项、自由文本和刷新后恢复。
- 混合询问与工具审批部分完成后，自动重新发布剩余 HITL，保持 turn 链路可继续。
- 增加 `screen_capture` 和 `computer_use` 工具，支持 Windows 桌面截图与鼠标键盘操作；Linux 当前支持 X11。
- 桌面操作要求多模态模型、近期截图和默认审批，增加坐标映射、媒体恢复、结果展示和跨平台编译支持。

### 修复与测试

- 修复工作组成员询问无法在前端输入和部分 HITL 恢复后不再出现的问题。
- 完善截图结果在会话刷新后的媒体重建，避免工具结果只在当前进程可见。
- Go、Python、Web UI、Tauri、Windows x86/x64 与 Linux 构建验证通过。
...
- 增加 AgentRef HITL 的结构化路由元数据和 `agent.turn.resume` Node→Manage→Node 回传链路，支持 Manage 重启后继续完成成员工具审批。
- 修复 AgentRef turn 在 HITL 暂停时被误判为成功完成的问题，并处理重复恢复消息的幂等情况。
- 修复工作组会话打开终端时未使用对应会话工具注册表的问题，避免远程成员终端回落到个人 Agent 作用域。
- 修复 UI 创建的终端被错误视为临时会话的问题；离开终端工作台现在只是分离，显式终止或权威关闭事件才会结束会话。

### 终端工作台

- 修复空白终端页点击事件误创建默认 PowerShell 终端的问题；必须先选择目标配置后才会创建会话。
- 增加当前终端类型、地址、用户和状态展示，以及终端操作菜单；完善远程目标信息回填和终端清单切换。
- 保持消息/终端状态栏入口、MCP/终端面板和输入区的现有布局行为，不引入未确认的面板定位变化。

### 测试

- 增加 AgentRef HITL 恢复、工作组会话工具作用域和 UI 终端 keep-alive/断线恢复回归测试。
- 前端生产构建、Web UI 单元测试、Node 工作组/终端 API 测试及 Manage 工作组测试通过。
