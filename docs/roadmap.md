...
以下按子域归纳现行 **Go 本地助手栈**；字段级契约见 [architecture/overview.md](./architecture/overview.md)、[architecture/agent-node-api.md](./architecture/agent-node-api.md) 与各包 **`README.md`**。
...
- **工具集**（**`node/internal/tools/`**）：清单见 [handbook/附录/内置工具参考.md](./handbook/附录/内置工具参考.md) 与 **`node/internal/tools/README.md`**。
