...
> **权威来源**：`node/internal/tools/registry.go` → `Definitions()`。  
...
- `prompt_context` 中的 soul/custom 与长期记忆会作为当前请求的运行时上下文注入，通常无需 `read_file`；用户称呼来自 Node 的 `PreferredName`，旧 `user.md` 仅用于迁移。
...
| 参数 | 类型 | 说明 |
|------|------|------|
| **`call_purpose`** | string | 一句话说明本次调用目的（UI 摘要首行，如 `bash(此内容)`） |
...
### 工具组（Agent `defaults.tools.enabled_groups`）
...
| **`browser`** | browser_run_task, browser_task_status, browser_task_cancel（须 `browser.enabled`；伴生 Agent） |
| **`wecom`** | wecom_send_markdown, wecom_send_file（须 `wecom.enabled` + webhook） |

未配置或 `[]` = 不启用任何工具组。企微组需进程级 `wecom` 配置。跨机器协作请用 **工作组（Workgroup）**。

### 执行形态

- Schema 均为**同步**；`bash_run` 到达 `timeout_seconds` 或默认硬上限后直接终止并返回 `TIMED_OUT`，不会转后台。UI 可手动终止；需要长期运行状态时使用 `terminal_open`。
...
**描述**：prompt_context 已作为当前请求的运行时上下文注入，通常无需 read。按行窗口读取文本文件，大文件用 line_offset/line_limit 分页。
...
**返回**：文件 mtime、编码、总行数、本页行区间、`next_line_offset`、是否因 token 上限截断等元数据 + 正文。若正文因 token 上限在某一行中间截断，`next_line_offset` 会指向该未完整读取的行；即使该行是文件最后一行，也应从该 offset 重新读取。
...
**描述（Linux/macOS）**：执行 bash 命令；cwd 省略时默认为工作区根。省略 shell_type 时默认 bash。command 须为 bash 语法（ls、grep、cat），勿用 PowerShell/cmd 语法。外置 CLI、编译二进制与 shell 脚本见 `externaltools/` 与 `externaltools_menu.md`。除非明确需要，否则避免 su/sudo。若达到 timeout_seconds，超时会终止进程并返回失败结果，不会转后台；省略时使用默认 600s 硬上限。长输出按 tools.bash_compress 清洗；超长落盘并在 history 头尾摘要。
...
|------|------|------|
| `command` | ✓ | shell 命令字符串 |
| `timeout_seconds` | | 1–600；超时直接终止并返回 `TIMED_OUT`，不会转后台 |
| `cwd` | | 执行目录（默认工作区根） |
...
**描述**：查询支持后台执行的工具任务状态与输出摘要。`bash_run` 始终同步执行，不会生成后台 job；其他后台任务完成通常会通过 `async_tool_result` 回灌。

| 参数 | 必填 | 说明 |
|------|------|------|
| `job_id` | ✓ | 后台工具返回的 job_id；`bash_run` 不会生成 |
...
**描述**：取消支持后台执行的工具任务；`bash_run` 不会生成后台任务。
...
**执行**：编排器在本地 turn 发 SSE **`hitl_required`**（`items[]` 中 `hitl_type=user_information`），Web UI 收集后 `resume(type=user_information)` 回灌。
...
**描述**：将选定 skills 加载到当前会话。可用 skill 目录以元数据形式出现在 system prompt；匹配到 skill 且尚未加载时，**先调用本工具**。整组**替换**当前已加载列表（非追加）；`skill_names=[]` 清空。SKILL.md 正文在本次变更后的下一个模型 Step 以独立的 `role=user` ContextInjection/context message 注入，不重复拼入 system prompt；修改文件由加载 hook 负责边界检查。
...
**描述**：从当前会话已加载 skills 中移除指定项；移除后该 skill 正文不再注入后续模型 Step 的 ContextInjection。
...
**描述**：清空当前会话已加载的全部 skills，等价于 `load_skills([])`；后续模型 Step 不再携带这些 skill 的 ContextInjection。
...
## 六、临时子 Agent 组（`child_agents`）

### `create_temporary_agent`

**源码**：`node/internal/tools/tool_childagent.go`

**描述**：创建同进程临时 Agent 执行自包含子任务。wait=true 时阻塞至完成。task 须完整上下文；allowed_tools 指定其可用工具子集（仅限父 Agent 当前拥有的工具）。skill_names 可在创建时预加载 skills（子 Agent 运行期不可再 load_skills）。
...
**描述**：等待一个或多个临时 Agent 到达终态并汇总。

| 参数 | 必填 | 说明 |
|------|------|------|
| `child_agent_ids` | ✓ | create_temporary_agent 返回的 session id 列表 |
...
进程级须 `browser.enabled: true`，并同机运行 `dagents-browser`。启用后 Node 为每个主 Agent 创建持久伴生 `{agent_id}-browser`；细粒度 DOM 动作在 sidecar `browser_use.Agent` 内闭环，不对主 Agent LLM 暴露。

### `browser_run_task`

**源码**：`node/internal/tools/browser_tool_task.go`

**描述**：向伴生 session 派发自然语言浏览器任务（默认同步等待完成并返回 summary）。

| 参数 | 必填 | 说明 |
|------|------|------|
| `task` | ✓ | 自然语言目标 |
| `max_steps` | | 上限步数（sidecar 默认约 50） |
| `wait` | | 是否等待完成（默认 true） |
| `wait_timeout_seconds` | | 等待超时（默认 300） |

**审批**：种子默认 `always`（可按 Agent 策略调整）。

### `browser_task_status`

**描述**：查询任务状态；结果扁平含 `summary` / `success` / `urls` / `screenshot_paths` 等。
...
设计：[browser-remote-service-mode-a.md](../../design/browser-remote-service-mode-a.md)。

---

## 企业微信组（`wecom`）

进程级需 `wecom.enabled: true` 并配置 Webhook（设置 › Node 能力，或 `config.yaml`）。对应企业微信「消息推送」API（[文档](https://developer.work.weixin.qq.com/document/path/99110)）。

### `wecom_send_markdown`

**源码**：`node/internal/tools/tool_wecom.go`

**描述**：发送 `markdown_v2` 消息到已配置的群消息推送。

| 参数 | 必填 | 说明 |
|------|------|------|
| `content` | ✓ | markdown_v2 正文（UTF-8，≤4096 字节） |
...
**描述**：上传工作区文件并发送 file 消息（内部完成 `upload_media` + send）。

| 参数 | 必填 | 说明 |
|------|------|------|
| `path` | ✓ | 工作区相对路径或绝对路径（≤20MB） |
...
| 名称 | 说明 |
|------|------|
| **`search_file`** | Registry handler 与 `grep_file` 行为一致，**不在** `Definitions()` 中 |
...
| [triggers README](../../../node/internal/triggers/README.md) | 触发器设计与存储 |
| [ux-agent-owned-file-approval.md](../../design/ux-agent-owned-file-approval.md) | 写盘信任链 |
| [tool-context-cost-analysis.md](../../archive/experiments/tool-context-cost-analysis.md) | WS3/WS6 上下文治理（历史实验） |
