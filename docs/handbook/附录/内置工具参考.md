...
> `background_job_status` / `background_job_cancel` 已从当前模型工具目录移除。旧会话中的后台任务回灌仍由 Node 内部兼容层处理；新任务请使用同步 `bash_run` 或显式的 `browser_run_task(wait=false)`。
...
| 名称 | 说明 |
|------|------|
| **`search_file`** | 旧 Registry handler，与 `grep_file` 行为一致，**不在** `Definitions()` 中 |
