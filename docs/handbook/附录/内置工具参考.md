...
未配置或 `[]` = 不启用任何工具组。企微组需进程级 `wecom` 配置。
...
**描述**：先用 `list_available_skills` 按需查询可见 skill 元数据，再将选定 skills 加载到当前会话。整组**替换**当前已启用列表（非追加）；`skill_names=[]` 清空。SKILL.md 正文在本次变更后的下一个模型 Step 以独立的 `role=user` ContextInjection/context message 注入；修改文件由加载 hook 负责边界检查。
...
**描述**：从当前会话已启用 skills 中移除指定项；移除后该 skill 正文不再注入后续模型 Step 的 ContextInjection。
...
**描述**：清空当前会话已启用的全部 skills，等价于 `load_skills([])`；后续模型 Step 不再携带这些 skill 的 ContextInjection。
