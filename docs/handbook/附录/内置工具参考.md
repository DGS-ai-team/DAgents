...
**描述（Linux/macOS）**：执行 bash 命令；cwd 省略时默认为工作区根。省略 shell_type 时默认 bash。command 须为 bash 语法（ls、grep、cat），勿用 PowerShell/cmd 语法。外置 CLI、编译二进制与 shell 脚本见 `externaltools/` 与 `externaltools_menu.md`。除非明确需要，否则避免 su/sudo。同步等待 timeout_seconds（默认 30）；超时自动降级后台 job，完成后自动回灌。长输出按 tools.bash_compress 清洗；超长落盘并在 history 头尾摘要。
