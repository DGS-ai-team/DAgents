...
- `prompt_context` 中的 soul/custom 与按 Turn 召回的 Memory Context 会作为当前请求的运行时上下文注入，通常无需 `read_file`；用户称呼来自 Node 的 `PreferredName`。
...
| `condition` | ✓ | 调度条件，三选一：<br>• `{"interval_seconds": N}` 周期<br>• `{"fire_at": unix秒}` 单次<br>• `{"schedule": {...}, "cmd": "可选 bash 条件脚本"}`<br>schedule 示例：daily `{kind,hour,minute}`；weekly `{kind,weekday,hour,minute}`（0=周日）；monthly `{kind,day,hour,minute}`（-1=月末）。cmd 可用于手动、周期、单次和日历触发，由目标 Agent 的 session/turn 经 hooks、policy、HITL 执行，exit 0 才投递 |
...
**描述**：创建同进程临时 Agent 同步执行自包含子任务，工具返回前等待子 Agent 进入终态。task 须完整上下文；allowed_tools 指定其可用工具子集（仅限父 Agent 当前拥有的工具）。skill_names 可在创建时预加载 skills（子 Agent 运行期不可再 load_skills）。
