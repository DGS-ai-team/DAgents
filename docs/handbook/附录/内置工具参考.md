...
**描述**：加载 skills 到当前会话，使后续 system prompt 注入已加载 skill 正文。可用 skill 目录位于 system prompt 尾部；任务与某 skill 匹配且尚未加载时，**必须先调用**。整组**替换**当前已加载列表（非追加）；`skill_names=[]` 清空。已加载内容即 SKILL.md 正文，修改文件不必重复 read skill.md。
