...
日常小改动见 [CHANGELOG.md](../../../CHANGELOG.md)。模块 API 见各包 `README.md` / `REFERENCE.md`。
...
**大型专题文**（如工具链成本、压缩 cache）另用四段结构：**背景与痛点 → 分析 → 优化思路 → 落地方案**（详见历史实验 [tool-context-cost-analysis.md](../../archive/experiments/tool-context-cost-analysis.md)）；细节实录写入本页，专题文保持可扫读。
...
- 完整分析：[context-compression-cache-analysis.md](../../design/context-compression-cache-analysis.md)
...
- [tool-context-cost-analysis.md](../../archive/experiments/tool-context-cost-analysis.md)（历史精简总览）
- [tool-before-hook-duplicate-approval.md](../../design/tool-before-hook-duplicate-approval.md)
- [skills-context-cost-analysis.md](../../archive/experiments/skills-context-cost-analysis.md)（WS4 搁置存档）
...
审批逻辑仍在 `processToolCalls` 内直接调用 `policy.DecideTool`，与 [agent-hooks.md](../../design/agent-hooks.md) 规划的 **`tool.before_each`** 未收敛。模型在短窗口内 **同名同参** 重复调用 tool（尤其 status 轮询）时，现网 HITL 仅有批准/拒绝，无法 **挂起后执行**，也无法在 Node 侧统一拦截。
