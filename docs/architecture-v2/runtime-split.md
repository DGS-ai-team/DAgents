...
Client Plane      用户交互：Python TUI / Go TUI / Web UI / API client
      │
      ▼
Control Plane     Backend API、session/connection、ProxyManager、策略、SSE routing
      │
      ▼
Brain Layer       LLM 推理、Agent turn loop、上下文、A2A reasoning
      │
      ▼
Execution Plane   Go Proxy、本地工具、shell、文件系统、宿主机能力
```

Brain Layer 和 Control Plane 都运行在 Python Backend 中，但职责不同：Brain 决定“要做什么”，Control Plane 决定“能不能做、交给谁做、如何追踪结果”。Execution Plane 不思考，只执行经过授权的任务。
...
- 维护 session、connection、Proxy presence 和执行任务状态。
- 根据 `agent_type` 和 Proxy 状态路由工具执行。
...
Execution Plane 由 Go Proxy 和 Backend 本地工具执行器组成。

### 4.1 Server Agent 本地执行

`agent_type: "server"` 的 Agent 保持现有行为：大脑和身体都在 Python Backend，工具在 Backend 本机执行。

### 4.2 Terminal Agent 远程执行

`agent_type: "terminal"` 的 Agent 使用 Go Proxy 作为身体。Go Proxy 部署在目标宿主机上，主动出站连接 Backend，并接收 Backend 通过控制通道下发的执行任务。
...
Client Plane 不直接访问 Go Proxy。所有用户输入先进入 Backend，由 Control Plane 和 Brain Layer 决定是否需要远程执行。
...
| 策略判定 | 是 | 执行本地硬约束 |
| shell 执行 | server agent 本地执行 | terminal agent 远程执行 |
| 文件读写 | server agent 本地执行 | terminal agent 限于 `fs_root` 执行 |
...
Brain Layer 产生 ToolCall
  │
  ▼
Control Plane 查询 agent_type、session、policy、proxy presence
  │
  ├── server agent   → Backend 本地工具执行器
  └── terminal agent → ProxyManager 通过 control channel 下发任务
```
