...
**状态（2026-07）**：规划定稿，按版本递进交付。  
**基线**：当前 **`v0.5.5`**（Browser 模式 A、Windows 安装向导、multimodal / `read_image`）。
...
| SSE / 待办 | F-E1–E4, **F-E10–E12** |
| 通知 / 打开 UI | F-N1–N3, F-U1–U3 |
...
⑤ F-E1–E4/E10–E12      Shell SSE、session 待办、鉴权
...
3. 手工开 UI 处理 HITL → Shell 待办消除  
4. 退出 Shell → Node 停止；二次启动仅一个 Shell  
5. idle session 压缩 + 卸内存 → 再打开 Create + hydrate 正常  
