...
**状态（2026-07）**：**`v0.6.0`–`v0.6.2`**、**`v0.7.0`–`v0.7.5` 已发布**。当前开发见 `[Unreleased]`。  
...
| **v0.7.0** | **跨端与体验收尾** | TUI hydrate、缩略图/lightbox、Node update 迁移、A2A hydrate、Shell 运维 | ✅ |
| **v0.7.3** | **Web UI 设置化** | 连接/压缩阈值 Web 配置、ContextMeter、OpenAI thinking、安装向导简化 | ✅ |
| **v0.7.4** | **稳定性 + 设置扩展** | ack/seq/focus 修复、`clearPartialToolIndex`、设置页全量配置（除 Node 地址） | ✅ |
| **v0.7.5** | **Composer + 设置页 UX** | 附件 icon 工具栏、设置页统一滚动 | ✅ |

```text
v0.5.5 ──► v0.6.0 ──► v0.6.1 ──► v0.6.2 ──► v0.7.0 ──► v0.7.3 ──► v0.7.4 ──► v0.7.5
           Shell+HITL   UI重构+     更新+路径    TUI+ polish   设置页+上下文环
           Hydrate      show_image   paste                      OpenAI thinking
           idle evict   + media
```
...
**v0.6.2 功能开发已完成**；打 tag 前跑 [`v0.6.2-smoke-checklist.md`](./v0.6.2-smoke-checklist.md)。
...
⑤ F-E5 / F-I5 / F-M8   运维与 TUI media 提示              ✅
```
...
| 2026-07 | **`v0.7.3` 发布**：Web UI 设置 › 连接/压缩阈值、ContextMeter、OpenAI thinking、Windows 安装向导简化 |
| 2026-07 | **`v0.7.4` 发布**：SSE ack/seq/focus 修复、`clearPartialToolIndex`、设置页全量配置（除 Node 地址） |
| 2026-07 | **`v0.7.5` 发布**：Composer 附件工具栏、设置页统一滚动布局 |
