...
| Auto 如何标识 | 名称旁固定文字徽标“Auto”，配独立图标；运行状态另列，颜色不是唯一标识 |
| 是否单独分组 | Agent 列表默认按“自主智能体 / 普通智能体”分组，可切换全部或工作目录视图；搜索贯穿分组 |
...
列表默认两组，自主智能体在前；组内优先保留用户固定项，再按交互活跃度排序。提供“全部 / Auto / 普通”筛选和“按类型 / 按工作目录”互斥分组选择，避免类型×目录多层嵌套。同一 Agent 同一列表只出现一次；筛选、分组和折叠状态按 Node 保存，不修改服务端实体归属。
