...
事实记忆候选通过来源与适用验证后提交。经验手册按 2026-09-09 用户明确要求改为文件系统：设置中指定手册根目录，模型可在该目录内自主创建层级、读取、修改和整理文件，不以数据库版本记录作为手册内容来源。模型已读取的上下文记录文件路径与内容摘要，避免运行中的文件变更静默改写已有模型请求。维护单独记录消耗且计入 Agent 总额度，与业务运行共享执行槽，业务优先。首版只支持记忆整理和手册修订，不支持自行安装工具或修改运行时代码。
...
默认事实记忆维护只写自己的 Agent scope。现有记忆服务同时可访问 global scope，不能因为服务支持全局记忆便让 Auto 维护自动改写它。手册默认根目录位于已有 `EnsureWorkspaceState` 的 Agent 私有命名空间，用户可在智能体设置中指定目录；目录内组织由模型自主维护，不强制单篇手册或固定层级。手册内容、索引与修改历史均用文件实现，不新增手册数据库；事实记忆继续使用现有存储。同工作目录员工默认仍分别维护，文件工具检查手册根目录边界及链接逃逸。界面呈现配置目录、目录树、文件内容与修改记录，按需读取内容，不将整棵目录每次全部注入模型。恢复旧文件内容保留修改历史，不回退已经发生的业务副作用。已有实验手册数据库不自动删除或作为新实现的隐式内容源，迁移须显式处理。

### 5.4 手册维护的恢复契约

手册文件仍是唯一内容来源。现有 Goals 持久化记录只保存执行身份、有限输入证据、阶段、用量和文件变更摘要，不能将手册正文或文件版本转存为数据库内容源。记忆抽取先保存输入证据及候选结果，再推进记忆游标；手册阶段引用实际持久化记录 ID，不在 API 中重新计算身份。

| 阶段 | 再次进入或重启后的行为 | 完成证据 |
| --- | --- | --- |
| 记忆已提交，手册尚未准备 | 使用持久化证据创建唯一子执行，预留额度 | 父子执行相互关联，输入可读 |
| prepared | 优先恢复该执行，复用已预留额度；不先抽取新记忆 | 唯一调用者原子领取并绑定会话 |
| running | 不自动重新调用模型；核对原会话、文件历史及用量 | 明确对账结果，不能仅因进程退出认定失败或成功 |
| recovery_required | 页面显示原因及待处理状态；需先完成对账 | 授权恢复动作与结果留痕 |
| completed | 跳过已完成输入，不再消耗模型额度 | 文件变更摘要、已知实际用量和完成阶段一起保存 |

手册仍有待处理输入时，单次批处理达到数量或时间上限应返回待继续，不能将每日维护标为完成。只有成功领取执行的调用者可以结算；停止维护后不得领取旧 prepared 执行。每个临时会话都须释放，释放执行槽后再同步清理会话，避免死锁。

验收必须包含：记忆游标已推进且没有新消息时恢复 prepared；全部剩余额度已预留时恢复；并发领取仅调用一次；running 重启不重跑；配置停用阻止领取；批量输入超过单次上限最终全部处理；未知用量不能显示完成。仅有阶段存储单测，不足以证明调度器重启恢复已完成；调度 occurrence 的恢复入口和界面必须一并验证。
