...
| 默认激活 | 已同步稳定 trigger、应用工具轮次上限及用户输入优先；重启仅冻结待恢复 trigger，双 Agent 隔离、关闭频率保留投递及恢复后重新同步通过 API 专项测试；恢复界面待视觉验收 |
| Dreaming | 同一 turn 的持久关联、连续 ASK、审批批准/拒绝/取消、经验原子提交、结束边界重置及 Ack 已接入；调度端到端和 Session 全包 race 通过；审批等待中重启、孤立记录与排队消息保留专项 race 已通过，真实提供商待验收 |
| 设置与总览 | Node 设置、Todo、Node/Manage dreaming 总览已接入；前端、Manage接收/持久化及隔离测试通过；最新运行环境视觉验收未完成 |
| 脚本条件 | 持久审批、Manager 创建/替换回调绑定及生产接线已实现；原 HTTP 审批和真实下游投递失败冻结/重开测试通过，最终运行环境验收未完成 |
| 旧架构清理 | 旧 Goal/Cycle/event/maintenance 生产路由与调度入口已退役，专属前端及配置已清理；残余未使用底层代码继续核查，不建立旧消息兼容层 |
| 最终验收 | 新版 Node/Manage 已启动，真实配置 LLM 聊天、默认激活、dreaming 提交及重置已有证据；重连、手册写入、工具轮次触顶、完整视觉仍需闭环，整体目标保持进行中 |

用户已恢复界面操作并指定内置浏览器。后续完成重连运行复验、真实手册写入与工具轮次触顶、trigger 恢复入口及 Node/Manage 全页桌面/窄屏视觉。旧工具生产注册已清理，剩余内部历史代码不建立新兼容层。不得仅凭编译、mock 工具测试或接口结构声明完整产品验收完成。
