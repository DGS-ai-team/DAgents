...
Dreaming 的 `recovery_pending` 表示经验提交后的上下文重置尚未确认，由 dreaming 调度恢复；它不属于 trigger delivery，不能链接到“丢弃旧触发投递”。默认唤醒的恢复入口必须读取该 Agent 默认 trigger 的 `recovery_required` 及原 delivery，不能从 dreaming 状态推断。
...
| 范围 | 当前证据与剩余工作 |
| --- | --- |
| 职责、Todo、单主会话 | 已接入实际 system prompt / 请求上下文及用户、模型共享 Todo；本地测试模型和隔离测试通过，真实提供商待验收 |
| 默认激活 | 已同步稳定 trigger、应用工具轮次上限及用户输入优先；重启仅冻结待恢复 trigger，双 Agent 隔离、关闭频率保留投递及恢复后重新同步通过 API 专项测试；恢复界面待视觉验收 |
| Dreaming | 每日调度、经验原子提交、活跃上下文重置、保留历史及重开恢复已接入；已确认 ASK 批准后尚未接回经验提交，正在补齐同一 dreaming turn 的持久关联与收尾，不能视为完整闭环 |
| 设置与总览 | Node 设置、Todo、Node/Manage dreaming 总览已接入；前端、Manage接收/持久化及隔离测试通过；最新运行环境视觉验收未完成 |
| 脚本条件 | 持久审批和 Manager 生产接线已实现，原 HTTP 审批入口专项测试通过；恢复 runtime 的回调绑定和真实下游投递失败测试仍在收尾，最终验收未完成 |
| 旧架构清理 | 旧 Goal/Cycle/event/maintenance 生产路由与调度入口已退役，专属前端及配置已清理；残余未使用底层代码继续核查，不建立旧消息兼容层 |
| 最终验收 | 尚未完成新版 Node/Manage 启动、真实配置 LLM、全页视觉及干净工作区检查，整体目标保持进行中 |

后续按顺序完成：脚本条件真实审批链路及端到端测试；验证 trigger 恢复界面入口；清理残余依赖和同步文档；启动新版环境完成真实模型及视觉验收。不得仅凭编译、mock 工具测试或接口结构声明生产链路完成。
