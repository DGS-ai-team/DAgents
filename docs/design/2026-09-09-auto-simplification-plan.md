...
| 最终验收 | 新版 Node/Manage 已启动，真实配置 LLM 聊天、默认激活、dreaming 提交及重置已有证据；少量标签页重连、真实手册写入和工具轮次触顶已验收；完整视觉、工作组 WS 鉴权与残留旧代码仍需闭环，整体目标保持进行中 |

用户已恢复界面操作并指定内置浏览器。后续完成 trigger 恢复入口及 Node/Manage 全页桌面/窄屏视觉；少量标签页重连、真实手册写入、工具轮次触顶和无工作静默已有验收证据，多标签页连接限制不扩大为通过。旧工具生产注册已清理，剩余内部历史代码不建立新兼容层。不得仅凭编译、mock 工具测试或接口结构声明完整产品验收完成。
