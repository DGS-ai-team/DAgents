...
| 最终验收 | 新版 Node/Manage 已启动，真实配置 LLM 聊天、默认激活、dreaming 提交及重置已有证据；少量标签页重连、真实手册写入和工具轮次触顶已验收；工作组 WS 鉴权已修复并通过真实连接复验，旧专属包已删除；完整视觉及默认 trigger 恢复入口的实际界面验收仍需闭环，整体目标保持进行中 |
