...
| Dreaming | 同一 turn 的持久关联、连续 ASK、审批批准/拒绝/取消、经验原子提交、结束边界重置及 Ack 已接入；调度端到端和 Session 全包 race 通过；审批等待中重启仍在专项复核，真实提供商待验收 |
...
后续按顺序完成：补齐审批等待期间重启的最终复核；启动新版环境完成真实模型验收；经用户恢复界面操作后验证 trigger 恢复入口及 Node/Manage 视觉。旧工具生产注册已清理，剩余内部历史代码不建立新兼容层。不得仅凭编译、mock 工具测试或接口结构声明完整产品验收完成。
