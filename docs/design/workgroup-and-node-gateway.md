...
> **状态**：**产品方向冻结；D0.5 契约已冻结** — 见 [`workgroup-d05-contracts.md`](./workgroup-d05-contracts.md)（§19 Verdict A）；**D0.9–D4 已完成**；**D5 进行中**（停广告 + peers/create 410 + peers 路由/RemoteScreen/Control create 客户端已拆；Edge/remote stub 运行时仍待）  
...
| **D5** | 拆除 Placement/Edge/remote stub/沙箱产品路径；拆旧 A2A — **进行中**（setup/registrar 停广告；Manage peers·create → 410；Node peers 路由与 RemoteScreen UI 已拆；DELETE 与 Edge/remote stub 仍保留） |
