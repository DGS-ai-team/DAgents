...
> **状态**：P2 Edge Tunnel 落地中（2026-07-29）  
> **分支**：`cursor/remote-agent-placement-7e3e`（大动作独立演进，允许通信面重构；**不合入 dev 直至评审通过**）  
...
| **P2** | Manage Edge Tunnel：远端 Agent 的 `/v1/agents/{id}/**` + messages/streams 统一代理 | **大**（通信面）✅ 本分支已落地骨架 |
| **P3** | 聊天/SSE/HITL 走 Edge 闭环（端到端验证与缺口修补） | 大 |
| **P4** | Screen 旁观 SSE | 中 |
| **P5** | Registry `node_id+agent_id`；A2A 寻址对齐 | 大 |

P2 是通信重构的核心：Manage `POST /v1/edge/sessions` + `ANY /v1/edge/{session}/proxy/...`；owner Node 对 `origin=remote` 在 Handler 入口统一 upgrade（DELETE 根路径仍走 Control 双删）。
