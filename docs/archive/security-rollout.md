...
> **已归档**。现行安全边界与运维实践见 [handbook/06-运维与案例](../handbook/06-运维与案例.md)、[04-能力与策略](../handbook/04-能力与策略.md)。下文含已拆除 A2A 验收项。
...
- A2A 入站：Manage inbox Task → `compliance_executor` 本地 turn；HITL 可经 Manage 中继到 caller Client。
...
- A2A 维度：Manage Task `trace_id`、`task_id`、caller/callee `agent_id`、TTL。
...
- Manage 记录 Agent TTL 与在线状态；查询时自动剔除过期 Agent。
...
- 若 A2A 入站引发兼容问题，可临时关闭 `manage.a2a.enabled` 或让调用方仅发普通文本 Task。
