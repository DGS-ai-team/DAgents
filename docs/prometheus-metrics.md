...
1. **LLM token 累计（Counter，`_total` 后缀）**  
   - **`dagents_llm_prompt_tokens_total{model=...}`**、**`dagents_llm_completion_tokens_total{model=...}`**  
   - **`dagents_llm_prompt_audio_tokens_total`**、**`dagents_llm_prompt_cached_tokens_total`**（来自 **`usage.prompt_tokens_details`**）  
   - **`dagents_llm_prompt_cache_hit_tokens_total`**、**`dagents_llm_prompt_cache_miss_tokens_total`**  
   - 在收到流式响应 **`usage`** chunk 时由 **`record_llm_token_usage(..., usage=原始 usage)`** 对本轮上报值 **`inc`**（与 **`include_usage`** 配置配合）。**约定**：`usage` 表示 **当前 completion 请求的消耗**；若某网关上报 **账号级终身累计**，会导致本进程 **重复累加**，需由网关或接入层改为按请求口径。
