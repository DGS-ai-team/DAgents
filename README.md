...
<p align="center">
  ...
  <a href="CHANGELOG.md"><img src="https://img.shields.io/badge/release-v0.2.14-20260607-green" alt="v0.2.14"></a>
...
**DAgents** 面向需要 **工具调用、人工审批（HITL）、会话持久化** 的 Agent 场景。当前版本（**v0.2.14**）以 **Go Agent Node** 为唯一运行时：单进程承载 LLM turn loop（**OpenAI 兼容 / DeepSeek** 等）、内置工具、SQLite 会话、skills、上下文压缩与 trigger 调度。
...
| `fs_root` | 工具沙箱根（默认 `./.runtime`，与运行时目录一致） |
...
| [CHANGELOG.md](CHANGELOG.md) | 版本变更（**v0.2.14**） |
...
| 项 | 说明 |
|----|------|
| **当前版本** | **v0.2.14**（2026-06-07，0.x 预览；tag `v0.2.14`） |
