...
- 多 Agent 协作通过 `app/harness/tools/agent_peer.py` 对接 `register_center/`。
...
- `tool.py`：工具统一入口（`tool` 装饰器 + 工具清单 + OpenAI tools 适配）。
...
- `bash` 工具的审批由 `tool.py` 统一策略函数判断；shell 细粒度规则使用 `命令首词=审批模式` 文件（always/never/rule）。
...
- token 指标：`dagents_llm_prompt_tokens`、`dagents_llm_completion_tokens`，以及 **`prompt_tokens_details` / `prompt_cache_*`** 派生的 **`dagents_llm_prompt_audio_tokens`**、**`dagents_llm_prompt_cached_tokens`**、**`dagents_llm_prompt_cache_hit_tokens`**、**`dagents_llm_prompt_cache_miss_tokens`**（Gauge，**`record_llm_token_usage(..., usage=…)`** 对提供商 **`usage`** 做 **`set`**）
- 会话上下文 Gauge：`dagents_session_context_messages_count`（由 **`refresh_session_context_metrics`** 维护）
...
更完整的机制说明与新增指标步骤见 **[prometheus-观测.md](./prometheus-观测.md)**。
...
- 可靠性治理：缺少系统化重试、死信、幂等投递与投递可观测链路。
- 安全与治理：Register Center / API 缺少鉴权、限流、追踪字段标准化。
- 观测能力：缺少更细粒度的会话级指标、工具耗时分布与错误分类看板。
- 性能能力：长会话历史增长后的裁剪/归档策略可进一步增强。

## 3.4 待补充

- 前端能力（独立仓库 [DAgentsUI](https://github.com/DGS-ai-team/DAgentsUI)）与后端的联调文档与示例。
...
- 最后看 `app/harness/tools/` 与 `register_center/`
