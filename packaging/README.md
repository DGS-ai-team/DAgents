...
| **`dagents-backend-*.{zip,tar.zip,deb,exe}`** | 完整后端运行时（api + register_center + cli + `.runtime` + 启动脚本）的便携包或安装包 |
...
| **`windows/`** | Windows 安装器资源：Inno Setup 脚本与安装后暴露的 `dagents.cmd` 命令入口（含 `chat` 交互终端）；`dagents.cmd` 会去掉 `%~dp0` 末尾 `\` 再 `pushd`，避免 `Program Files\...\Backend\` 触发 CMD 解析错误 |
