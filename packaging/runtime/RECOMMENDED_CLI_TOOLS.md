...
DAgents **发布包不内置**第三方 CLI。下列工具经社区验证，适合与本机 Agent 配合使用；请自行下载安装，并确保可执行文件在 **`PATH`** 或 **`<运行根>/.runtime/scripts/`** 中。

安装后可将 **`.runtime/scripts`** 加入 `PATH`（Linux **`install.sh`** / Windows 安装包已默认配置），便于 Agent 通过 `bash_run` 调用。
...
- 用户自行安装的工具：放入 `.runtime/scripts/` 并更新 **[`scripts_menu.md`](scripts_menu.md)**（Agent 查阅索引）。
