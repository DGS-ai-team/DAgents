...
每步 `BuildSystemPrompt` 会调用 `RenderLoadedSection`，仅加载已加载 skills 的正文；启用 skills 工具组时，可用 catalog 元数据追加到 system prompt 尾部。tool loop 中 `load_skills` 使用详细加载结果并按需读取正文。
...
`load_skills` 会立即更新 session 状态和 hooks；正在进行的模型请求保持不变，但显式变更会在下一个模型 Step 创建新的 context segment，使正文立即可用。活动 Turn 中若发现正文边界摘要变化，会返回 `catalog_changed`；普通磁盘变化在下一次 human Turn 创建新 view。工具结果会同时返回请求、成功、拒绝和两个生效边界。
