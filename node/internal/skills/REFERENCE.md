...
| `(c *Catalog) RenderMetadataSection` | method | 将 context-boundary 的目录元数据渲染为 system prompt 片段 |
| `(c *Catalog) RenderLoadedSection` | method | 正文文本投影（仅诊断/测试；模型运行时使用独立 skill context message） |
