...
| 文件 | 说明 |
|------|------|
| `sqlite.go` | `SQLiteStore`：`agent_runtimes` 行读写、`messages_json`、`loaded_skills_json` |
| `runtime_state.go` | `RuntimeState`：InputBox checkpoint、Hook 状态与通知游标 |
...
默认文件路径为 `{runtime}/memory/sessions.db`。该库属于 Node 控制面，
不随 Agent workspace 移动；workspace 可能被多个 Agent 共享，Agent 记录通过 `agent_id` 隔离。

## 边界

- **生命周期热状态**在 `turn.Coordinator` 内存；`persist` 时写入消息快照与运行时游标。
- **JSONL 审计**在当前 Agent workspace 的 `.dagents/<agent_id>/history/`，与本包并行、不替代。
- `turn_events` 是 Turn/Step 生命周期事实源；`RuntimeState` 不复制 Turn/Step 状态。
