...
const defaultChildSystemPrefix = "你是临时任务助手，在限定工具内完成父 Agent 委派的自包含任务；不要向用户追问。"
...
// ParentDelegatableTools 父 Agent 可下放给临时 Agent 的工具名。
...
// IsParentOnlyTool 为仅父 Agent 可用的工具（含临时 Agent 管理工具，非 A2A）。
