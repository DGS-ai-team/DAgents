...
func formatBackgroundJobDoneLocked(job *backgroundJob) string {
	return strings.Join([]string{
		fmt.Sprintf("[TOOL_BACKGROUND_DONE] tool_name=%s job_id=%s status=%s", job.toolName, job.id, job.status),
		"---",
		job.result,
	}, "\n")
}
