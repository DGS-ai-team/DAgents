...
| `encodeFileContentRaw` / `encodeTextToLegacyChinese` | 写盘编码；GBK 失败回退 GB18030，再失败按 rune 替换 `?` |
...
| `execReadFile` / `execWriteFile` / `execSearchReplace` / `execGlobFiles` / `execGrepFile` / `execGrepFiles` | 内置实现 |
...
| `ParseToolCallArguments(arguments)` | 剥离仅供界面展示的 `call_purpose` |
...
| `fs_helpers.go` | `textSuffixes`、`isTextReadable`、`windowFromTotal`、`clipReadBodyToTokenBudget`、`mergeLineRanges` 等 |
