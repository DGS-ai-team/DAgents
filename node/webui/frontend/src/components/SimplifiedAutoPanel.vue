...
<template>
  <section class="simplified-auto-panel" aria-label="Auto 设置">
    ...
    <template v-else>
      ...
      <div class="simplified-auto-panel__row"><div><strong>每日 dreaming</strong><small>整理经验和手册，默认关闭</small></div><div class="simplified-auto-panel__control"><input v-model="draft.dreaming_enabled" type="checkbox" aria-label="启用每日 dreaming" /><input v-if="draft.dreaming_enabled" v-model="draft.dreaming_time" type="time" aria-label="dreaming 时间" /></div></div>
      ...
      <button class="btn btn--primary" :disabled="saving || !loaded" @click="save">{{ saving ? "保存中…" : "保存 Auto 设置" }}</button><p v-if="notice" role="status">{{ notice }}</p>
    </template>
  </section>
</template>
<style scoped>
