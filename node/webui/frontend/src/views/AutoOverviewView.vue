...
const items = ref([]);
const counts = ref({}); const total = ref(0); const page = ref(1); const pageSize = ref(20);
const search = ref(""); const status = ref(""); const workspace = ref("");
const appliedSearch = ref(""); const appliedStatus = ref(""); const appliedWorkspace = ref("");
let requestSeq = 0; let disposed = false;
...
async function load() {
  ...
  try {
    ...
    counts.value = result?.counts || {}; total.value = Number(result?.total || items.value.length); page.value = Number(result?.page || page.value); pageSize.value = Number(result?.page_size || pageSize.value);
    loaded.value = true;
  } catch (e) {
    if (disposed || seq !== requestSeq) return;
    error.value = e?.message || "无法加载 Auto 总览";
...
function openSettings(item) { router.push({ name: "settings-agent-detail", params: { agentId: item.agent_id }, query: { section: "autonomy" } }); }
...
function nextPage() { if (page.value * pageSize.value < total.value) { page.value += 1; void load(); } }
function prevPage() { if (page.value > 1) { page.value -= 1; void load(); } }
onMounted(() => { void load(); refreshTimer = window.setInterval(() => void load(), 30000); });
...
<template>
  <div class="app__body app__body--chat-v61 auto-overview-shell">
    ...
    <main class="app__main-col auto-overview" aria-labelledby="auto-overview-title">
      ...
      <section class="auto-overview__filters" aria-label="筛选"><input v-model="search" type="search" placeholder="搜索 Auto Agent" aria-label="搜索 Auto Agent" @keydown.enter="changeFilter" /><select v-model="status" aria-label="状态筛选" @change="changeFilter"><option value="">全部状态</option><option value="working">工作中</option><option value="activation_off">自主激活关闭</option><option value="standby">待命</option><option value="needs_attention">需处理</option></select><input v-model="workspace" type="search" placeholder="工作目录" aria-label="工作目录筛选" @keydown.enter="changeFilter" /><button type="button" class="auto-overview__filter-button" @click="changeFilter">筛选</button></section>
      ...
      <div v-else-if="!items.length" class="auto-overview__empty">暂无 Auto Agent</div>
