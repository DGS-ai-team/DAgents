...
def append_openai_message_with_journal(ctx: OpenAIConversationContext, message: dict[str, Any]) -> None:
    ...
    逻辑：
    1. **`copy.deepcopy(message)`** 得到插入瞬间快照（与后续列表内字典原地改动解耦）；
    2. **`ctx.messages.append(message)`**；
    3. **`record_raw_openai_message_append`**。
...
def insert_openai_message_with_journal(ctx: OpenAIConversationContext, index: int, message: dict[str, Any]) -> None:
    ...
    逻辑：
    1. **`deepcopy`** 快照；
    2. **`ctx.messages.insert(index, message)`**；
    3. **`record_raw_openai_message_append`**（按插入顺序追加到 JSONL，而非列表下标）。
