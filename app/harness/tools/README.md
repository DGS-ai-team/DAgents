...
| 文件 | 说明 |
|------|------|
| **`tool.py`** | 工具统一入口：`tool` 装饰器 + `get_tools()` + OpenAI tool 适配（`build_openai_toolkit`/`parse_tool_arguments`/运行时 args_schema 校验/结构化审批决策）；**当前模型可见工具清单**见 [built-in-tools.md](../../../docs/built-in-tools.md) |
