...
@tool("load_skills")
def load_skills(skill_ids: list[str] | None = None, context: OpenAIConversationContext | None = None) -> str:
    """使用场景：当模型希望显式加载会话技能时调用；不需要时可不调用。

    字段说明：
    - skill_ids: 可选技能 ID 数组；为空时不加载任何技能。
    - context: 会话上下文（可选，由 runtime 自动注入；模型无需手动提供）。
