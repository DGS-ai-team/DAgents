...
- **`agent_peer.py`**：公开工具入口，保留 **`agent_discover` / `agent_send_message` / `agent_broadcast` / `agent_peer_approve_tools`** 注册面；调用方 submit 与终态写入 A2A metrics。
- **`agent_peer_common.py`**：公共模型与无状态 helper：**`PeerApprovalEntry`**、**`PeerStreamSummary`**、SSE 回放汇总（带 **`Last-Event-ID: -1`**）、A2A token header、错误信封、resume 决策构造、任务状态映射；peer stream 写入 A2A metrics。
- **`agent_peer_registry.py`**：Register Center 解析与目录缓存：按分组发现、TTL 缓存、目标 Agent 解析、Agent Card 摘要补充；只读 HTTP 请求支持有界 transient retry；发现与 card 读取写入 A2A metrics。
