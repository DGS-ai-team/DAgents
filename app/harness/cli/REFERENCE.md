...
- **`_run_http_cli`**：HTTP CLI 主循环；使用固定 `client_id` 建立全局 SSE（按 `session_id` 过滤）；**`pending_stream_count`** 记录「已 submit、尚未消费完成」的队列数量；**`start_next_stream_from_pending`** 在当前无在跑消费 task 时按计数启动下一轮 **`_consume_stream_print`**；**`submit_user_text(..., interrupt=False)`** 在有在途 SSE 时只入队不打断；**`interrupt=True`**（**`/cancel …`**）先入队再 **`cancel_current_turn`** 并接续 pending；**`handle_cancel`**（**`/cancel`**）仅 cancel 后同样 **`start_next_stream_from_pending`**。
