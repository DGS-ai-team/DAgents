...
- **`ContextMessagePreview`** / **`SessionContextResult`**：**`GET /v1/sessions/{session_id}/context`** 响应模型（当前 context 摘要与最近消息预览）
- **`SkillMutationIn`** / **`SessionSkillsResult`**：session skills API 请求/响应模型（查询、加载、卸载当前会话 skills）
