...
- **`Settings`**：**Pydantic `BaseModel`（frozen）**；全局配置（含 LLM、**`summary_compression_silent_trigger_tokens`**、**`summary_compression_blocking_trigger_tokens`**、**`llm_stream_include_usage`**、**`metrics_enabled`**、**`app_log_level`**、队列（**`max_queue_size`** / **`agent_max_active_session_queues`** / **`agent_session_idle_evict_seconds`**）、**`fs_tool_read_max_bytes`** / **`fs_tool_search_max_bytes`**、本地调试相关配置、**`agent_id`**、**`registry_url`**、**`discovery_groups`**、**`agent_public_base_url`**、**`agent_peer_*`**、**`agent_session_store_enabled`**、**`agent_raw_message_history_enabled`** 等）
