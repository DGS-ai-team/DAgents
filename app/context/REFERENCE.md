...
- 枚举：
  ...
  - `SummaryCompressionPhase`（`not_started` / `idle` / `preparing` / `summarizing`）
- 模型：
  - `MessageRecord`：单条消息记录（`role/content/meta`）
  - `ConversationContext`：可持久化会话上下文（`history/openai_messages/pending_tool_calls/run_turn_phase/summary_*/tool_loop_count`）
  - `PendingToolCall`：待执行/待审批工具规格
  - `OpenAIConversationContext`：runtime 可变推理上下文（`messages/pending_tool_calls/run_turn_phase/summary_*/tool_loop_count`）
- 辅助函数：
