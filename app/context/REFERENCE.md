...
- 模型：
  ...
  - `ConversationContext`：可持久化会话上下文（`history/openai_messages/pending_tool_calls/run_turn_phase/messages_total_tokens/tool_loop_count`）
  - `PendingToolCall`：待执行/待审批工具规格
  - `OpenAIConversationContext`：runtime 可变推理上下文（`messages/pending_tool_calls/run_turn_phase/messages_total_tokens/tool_loop_count`）
- 辅助函数：
