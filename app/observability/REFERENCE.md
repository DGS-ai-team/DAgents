...
- **`LLM_PROMPT_AUDIO_TOKENS`**、**`LLM_PROMPT_CACHED_TOKENS`**、**`LLM_PROMPT_CACHE_HIT_TOKENS`**、**`LLM_PROMPT_CACHE_MISS_TOKENS`**：`usage` 中 prompt 明细/cache 快照（**`record_llm_token_usage(..., usage=...)`**）
