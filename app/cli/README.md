...
| [`version_info.py`](version_info.py) | `fetch_node_health`（版本以 Node `/health` 为准）、用户名解析 |
...
- **进入欢迎区**：连接成功后 `build_welcome_panel()` 写入 Rich `Panel`；标题与 `version` 行来自 **`GET /health`**（非 Client 独立版本号）。
