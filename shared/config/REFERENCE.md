...
| `Config` | `struct` | 根配置：agent_id、listen、local、llm、manage、data_dir 等 |
...
| `SkillsConfig` | `struct` | skills 开关、`root`、`max_in_prompt` |
| `CompressionConfig` | `struct` | `silent_trigger_tokens`、`blocking_trigger_tokens`（`<=0` 关闭对应档位） |
| `TriggersConfig` | `struct` | `enabled`、`poll_seconds`、`store_path` |
