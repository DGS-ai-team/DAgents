...
func FormatChildAgentsList(items []nodeapi.ChildAgentListItem, awaitingApproval map[string]bool) string {
	...
	for i, it := range items {
		...
		fmt.Fprintf(&b, "     purpose=%s template=%s status=%s\n", purpose, tmpl, status)
