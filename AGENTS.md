...
| Desktop tray (`desktop/tray`) | Skip on Linux | Windows-only |
...
- **Config:** `cp packaging/agent-client/config.example.yaml packaging/agent-client/config.yaml` (gitignored). Example ships `llm.mock: false` + `provider: deepseek` + `base_url: https://api.deepseek.com`. For keyless smoke tests set `llm.mock: true`. For real DeepSeek, keep `mock: false` and ensure `OPENAI_API_KEY` is in the **Node process environment** (the Node reads `llm.api_key_env`, default `OPENAI_API_KEY`).
